...
local Window = WindUI:CreateWindow({
	...
	User = {
        ...
        Callback = function()
            ...
            -- Mặc định nếu chưa có nhân vật
            local hp, maxHp = 0, 0
            local en, maxEn = 0, 0

            -- Kiểm tra nhân vật để lấy Máu và Năng lượng
            if char and char:FindFirstChild("Humanoid") and char:FindFirstChild("Energy") then
                hp = math.floor(char.Humanoid.Health)
                maxHp = math.floor(char.Humanoid.MaxHealth)
                en = math.floor(char.Energy.Value)
                maxEn = math.floor(char.Energy.MaxValue)
            end
            ...
            WindUI:Popup({
                ...
                Content = string.format(
                    "⭐ Level: %s\n❤️ Health: %s / %s\n⚡ Energy: %s / %s\n💰 Beli: %s\n🍎 Fruit: %s\n🧬 Race: %s\n☠️ Bounty: %s",
                    level, hp, maxHp, en, maxEn, beli, fruit, race, bounty
                ),
                Buttons = {
                    {
                        Title = "Đóng",
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("Minh Bell Hub New") then
			makefolder("Minh Bell Hub New");
		end;
		if not isfolder("Minh Bell Hub New/Blox Fruits/") then
			makefolder("Minh Bell Hub New/Blox Fruits/");
		end;
		if not isfile(("Minh Bell Hub New/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("Minh Bell Hub New/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("Minh Bell Hub New/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveSetting = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("Minh Bell Hub New/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("Minh Bell Hub New/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
spawn(function()
    game:GetService("RunService").Heartbeat:Connect(function()
        -- ĐIỀU KIỆN CHÍNH: Phải BẬT CẢ "Spin Position" VÀ "Auto Farm" thì mới xoay
        if _G.Settings and _G.Settings.Setting and _G.Settings.Setting["Spin Position"] == true 
           and _G.Settings.Main and _G.Settings.Main["Auto Farm"] == true then
            ...
            pcall(function()
                ...
                if Root then
                    -- 1. Xoay thân mình (Body Spin giống Redz)
                    Root.CFrame = Root.CFrame * CFrame.Angles(0, math.rad(20), 0)
                    
                    -- 2. Xoay quỹ đạo bay vòng tròn (Orbit)
                    local radius = 20
                    local farmDistance = _G.Settings.Setting["Farm Distance"] or 20
                    SpinAngle = (SpinAngle + 5) % 360
                    local radian = math.rad(SpinAngle)
                    
                    Pos = CFrame.new(math.cos(radian) * radius, farmDistance, math.sin(radian) * radius)
                end
            end)
        else
            -- KHI TẮT FARM (Dù nút Setting vẫn bật): Trả về vị trí đứng yên trên đầu quái
            if _G.Settings and _G.Settings.Setting then
                -- Nếu vẫn đang trong trạng thái farm nhưng tắt xoay hoặc ngược lại
                -- thì giữ vị trí trung tâm để không bị bay loạn
                Pos = CFrame.new(0, _G.Settings.Setting["Farm Distance"] or 20, 0)
            end
            SpinAngle = 0
        end
    end)
...
-- Hàm kiểm tra trạng thái Farm dựa trên cấu hình g setting.lua[cite: 5]
local function FarmAtivo()
    -- ƯU TIÊN TUYỆT ĐỐI (ELF)
    if _G.FarmPriorityElf or _G.FarmElfLevelCustom then
        return true
    end

    -- Nếu các bảng cài đặt chưa tải xong thì không chạy[cite: 5]
    if not _G.Settings.Main then return false end

    -- Trả về true nếu bất kỳ tính năng farm nào được bật
    return _G.Settings.Main["Auto Farm"] or 
           _G.Settings.Farm["Auto Farm Bone"] or 
           _G.Settings.Farm["Auto Elite Hunter"] or
           _G.Settings.Items["Auto Soul Guitar"] or
           _G.FarmMastery_S or _G.FarmMastery_G
end

-- Biến kiểm soát nội bộ
local _B = false

-- Hàm lọc quái Boss/Raid để tránh lỗi script
...
-- Hàm BringEnemy chính (Sử dụng Tween mượt mà)
BringEnemy = function()
    -- Chỉ chạy nếu FarmAtivo và Bring Mob trong Settings được bật[cite: 5]
    if not FarmAtivo() or not _B or not _G.Settings.Setting["Bring Mob"] then return end

    local plr = game.Players.LocalPlayer  
    local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")  
    if not hrp then return end  

    -- Cập nhật SimulationRadius để có quyền điều khiển quái
    pcall(function()  
        sethiddenproperty(plr, "SimulationRadius", math.huge)  
    end)  

    -- Điểm gom là vị trí quái mục tiêu (PosMon) hoặc vị trí nhân vật
    ...
    for _, mob in ipairs(enemies) do  
        ...
        -- Kiểm tra điều kiện quái và khoảng cách
        if hum and root and hum.Health > 0 and not IsRaidMob(mob) then  
            local dist = (root.Position - (typeof(targetPos) == "CFrame" and targetPos.p or targetPos)).Magnitude  

            if dist <= _G.BringRange and not root:GetAttribute("Tweening") then  
                count += 1  
                root:SetAttribute("Tweening", true)  

                -- Gom quái về vị trí đích bằng Tween
                local goal = typeof(targetPos) == "CFrame" and targetPos or CFrame.new(targetPos)
                local tween = TweenService:Create(root, TweenInfoBring, { CFrame = goal })  
...
-- Bộ điều khiển vòng lặp gom quái (Chạy song song)
task.spawn(function()
    while task.wait(0.01) do
        if FarmAtivo() and _G.Settings.Setting["Bring Mob"] then
            _B = true
            BringEnemy()
            task.wait(3) -- Thời gian gom
            _B = false  
            task.wait(2) -- Nghỉ ngắn để giảm lag
        else  
            _B = false  
            task.wait(1)  
        end  
...
local BringList = {"Low", "Normal", "High", "Custom"}
Tabs.SettingsTab:Dropdown({
    Title = "Bring Mob Mode",
    Value = _G.Settings.Setting["Bring Mob Mode"] or "Normal",
    Values = BringList,
    Callback = function(option)
        _G.Settings.Setting["Bring Mob Mode"] = option
        -- Cập nhật Range dựa trên lựa chọn
        if option == "Low" then 
            _G.BringRange = 150
        ...
        elseif option == "Custom" then
            -- Nếu là Custom, lấy giá trị từ ô Input đã lưu
            _G.BringRange = _G.Settings.Setting["Custom Value"] or 235
        end
    end
})

-- Ô nhập khoảng cách tùy chỉnh
Tabs.SettingsTab:Input({
    Title = "Custom Bring Distance",
    Placeholder = "Nhập số...",
    Callback = function(Value)
        local num = tonumber(Value)
        if num and num > 0 then
            _G.Settings.Setting["Custom Value"] = num
            -- Nếu đang ở chế độ Custom thì mới áp dụng số vừa nhập
            if _G.Settings.Setting["Bring Mob Mode"] == "Custom" then
                _G.BringRange = num
            end
        end
    end
})
-- Đoạn điều khiển Bring Mob của bạn sau khi đã sửa để gọi hàm BringEnemy
spawn(function()
    while task.wait() do
        -- Kiểm tra xem nút Bring Mob trên UI có bật không
        if _G.Settings.Setting["Bring Mob"] then
            pcall(function()
                -- Thay vì viết code gom quái dài dòng ở đây, ta chỉ cần gọi hàm đã viết
                BringEnemy() 
            end)
        end
    end
...
Tabs.LocalPlayerTab:Section({  Title = "Local Player", TextXAlignment = "Left" })
...
function CheckMonRaids()
    ...
    for i = 1, #Enemies do
        ...
        if v:FindFirstChild("HumanoidRootPart") then
            ...
            if dist <= 300 then 
                return true -- Trả về true nếu thấy dù chỉ 1 con ở gần 
            end
        end
    end
    return false -- Chỉ trả về false khi đã quét sạch folder mà không có con nào 
end;
...
Hakibutton = Tabs.MiscTab:Button({
	...
	Description = "",
	Callback = function()
		if _G.Settings.Misc["SelectStateHaki"] == "State 0" then
			replicated.Remotes.CommF_:InvokeServer("ChangeBusoStage",0)
		elseif _G.Settings.Misc["SelectStateHaki"] == "State 1" then
			replicated.Remotes.CommF_:InvokeServer("ChangeBusoStage",1)
		elseif _G.Settings.Misc["SelectStateHaki"] == "State 2" then
			replicated.Remotes.CommF_:InvokeServer("ChangeBusoStage",2)
		elseif _G.Settings.Misc["SelectStateHaki"] == "State 3" then
			replicated.Remotes.CommF_:InvokeServer("ChangeBusoStage",3)
		elseif _G.Settings.Misc["SelectStateHaki"] == "State 4" then
			replicated.Remotes.CommF_:InvokeServer("ChangeBusoStage",4)
		elseif _G.Settings.Misc["SelectStateHaki"] == "State 5" then
